<script>
    ...
    import { token, userProfile, matiere, getStoreValue, students, classes, schedule } from "../../../../stores/auth";
    ...
    async function fetchSchedule(classe){
        const response = await fetch(`http://localhost:8000/school/scheduleAPIView/${user.school_name}/${classe}`);
        const data = await response.json();
        schedule.set(data)
    }

    function handleClasse(classe) {
        fetchSchedule(classe);
        selected = true;
    }
    ...
    <div class="content-body">
        ...
        {:else}
                ...
                <div class="content-post">
                    <div class="add">
                        <button>
                            <a href="/school/home/schedule/add">
                    ...
                    <div class="container">
                        <h3>Liste des classes existants</h3>
                        <ul>
                            {#each user.school_classes as classe}
                                <li class="profil" on:click={handleClasse(classe)}>
                                    {classe}
                                </li>
                            {/each}
                        </ul>
                        ...
                        {#if selected}
                            <ul>
                                {#if $schedule.length > 0}
                                    ...
                                    <h2>Emploi du temps {sch.classe}</h2>
                                        <li>
                                            {@html sch.schedule.replace(/\n/g, '<br>')}
                                        </li>
                                    {/each}
                                {:else}
                                    Rien
